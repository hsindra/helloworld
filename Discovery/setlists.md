...
Objetivo: um **setlist** é uma lista nomeada de músicas já salvas, numa
ordem definida por quem monta. Ao abrir um setlist, a tela mostra as letras
...
Exemplo dado: setlist **"culto 02/08/26"** contendo **Só Tu És Santo** e
**Eu Vou Construir**, nessa ordem.

## Decisões confirmadas

- Botão **"Setlists"** ao lado do botão **"Minhas músicas"** (mesma linha,
  abaixo da caixa de busca).
- Clicar em "Setlists" mostra a lista de setlists existentes + um botão
  **"Criar setlist"**.
- Criar um setlist:
  - campo de texto pro **nome do setlist**;
  - campo de texto pra **buscar música salva** por nome + botão de lupa;
  - ao achar, clica em **"Adicionar"** pra incluir no setlist sendo
    montado (múltiplas músicas, uma de cada vez);
  - **"Salvar"** grava o setlist, que passa a aparecer na lista.
- Abrir um setlist mostra uma tela parecida com a visualização de música
  ...
  foram adicionadas.
- Dentro do setlist aberto: botão **"Ordenar"** (reordenar as músicas) e
  botão **"Adicionar"** (incluir mais músicas sem recriar o setlist).
- **Referência, não cópia:** o setlist guarda os `id`s das músicas salvas
  (como `viewerMeta.id` já faz hoje). Editar a música original reflete
  automaticamente no setlist; apagar a música original faz o setlist
...
- **Apagar já na v1:** apagar um setlist inteiro (na lista de setlists)
  e remover uma música individual de dentro dele (na tela de ordenar).
- **Tom por música + toggle único do setlist** (refinamento do pedido
  original — ver seção seguinte).

### Pré-requisito já implementado: tom preferencial na música individual

Antes dos setlists, o conceito de **tom preferencial** já foi construído
na tela de música individual (`app/page.tsx`) — os setlists reaproveitam
...
Isso já resolve, pros setlists, a pergunta "qual tom uma música toca por
padrão" — é sempre `song.preferredKey`, não o `song.key` original.

### Tom preferencial por música + toggle Graus/Tom do setlist

Cada música dentro de um setlist tem seu próprio **tom preferencial pro
setlist** — o tom em que ela será tocada *naquele evento específico*
...
- vem **pré-preenchido com o tom preferencial já salvo na música**
  ...
  um preferencial definido) quando ela é adicionada ao setlist — não o
  tom original;
- fica salvo como parte do item do setlist (persiste — não é só sessão),
...
Além disso, o setlist tem **um único seletor global "Graus" / "Tom"** no
...
- **Tom**: cada música renderiza no seu **tom preferencial de setlist**
  (não um tom único pra todas).

**Independente do modo**, toda música no setlist mostra uma **tag fixa**
...
*(estado vazio: "Nenhum setlist criado ainda.", mesmo padrão do estado
vazio de "Minhas músicas". `[🗑]` apaga o setlist, com confirmação.)*
...
- A busca filtra entre as músicas salvas (mesmo fuzzy-match de
  ...
  busca).
- "Adicionar" empilha na lista "Músicas no setlist", na ordem clicada,
...
- "Salvar setlist" fica desabilitado sem nome ou sem nenhuma música.

### 4. Setlist aberto (visualização corrida, somente leitura)
...
- `(Graus | Tom)` é o toggle único do setlist. Controla só como os
  **acordes** renderizam: "Graus" mostra o corpo em Nashville Number
  ...
  setlist. A tag `Tom: [E ▾]` **não depende do modo** — fica sempre
...
- Sem toggle "código ChordPro", sem menu de hambúrguer por música (isso já
  ...
  dois controles do setlist (Ordenar, Adicionar) e a tag/seletor de tom.

### 5. Ordenar músicas do setlist (arrastar e soltar)
...
- `[x]` remove a música do setlist direto dessa tela.
...
### 6. Adicionar música a um setlist existente

Reaproveita a mesma UI de busca+adicionar da tela de criação (seção 3),
aberta como uma tela/painel sobre o setlist já salvo — sem precisar
...
- **`lib/store.ts`** — novo tipo e CRUD espelhando o padrão de `SavedSong`:
  ...
  interface SetlistItem {
    songId: string;
    /** Tom em que essa música será tocada nesse setlist específico.
     ...
     * dos acordes quando o setlist está no modo "Tom". */
    preferredKey: string;
  }
  ...
  interface Setlist {
    id: string;
    name: string;
    items: SetlistItem[]; // ordem = ordem de exibição
    createdAt: number;
  }
  ```
  `SETLIST_INDEX_KEY = 'setlists:index'`, chave `setlist:{id}`, mesmas
  funções (`saveSetlist`, `listSetlists`, `getSetlist`,
  `deleteSetlist`) no mesmo estilo de `saveSong`/`listSongs`/etc.
- **`app/api/setlists/route.ts`** — `GET` (lista) / `POST` (cria, recebe
  `name` + `items`).
- **`app/api/setlists/[id]/route.ts`** — `GET` (detalhe, já resolvendo os
...
- **`app/page.tsx`** — estender `Mode` para incluir `'setlists'`; estado
  novo pro setlist aberto (`openSetlist`, modo `'graus' | 'tom'` local
...
- **Novo componente `app/SetlistView.tsx`** — recebe os `SetlistItem[]`
...
- Setlist com 0 músicas — permite salvar, mostrando estado vazio
  ("Nenhuma música neste setlist ainda — toque em Adicionar.") na tela de
  visualização.
- Música referenciada no setlist foi apagada de "Minhas músicas" depois —
  mostrar um espaço reservado ("Música removida") no lugar dela, sem quebrar
  o restante do setlist; o seletor de tom dela some junto.
- Busca sem resultado nenhum na tela de criar/adicionar.
- Adicionar a mesma música duas vezes no mesmo setlist — permitido (pode
  fazer sentido repetir uma música num culto), cada ocorrência com seu
  próprio `preferredKey` independente.
- Nome do setlist vazio — bloqueia "Salvar setlist".
- Nome de setlist duplicado — permitido (mesmo padrão de "Minhas
  músicas", que também não bloqueia títulos repetidos).

## Verificação

- Criar o setlist do exemplo ("culto 02/08/26" com Só Tu És Santo + Eu Vou
...
- Trocar o toggle Graus/Tom e confirmar que os acordes mudam de
  ...
  modos.
- Trocar o tom preferencial de uma música dentro do setlist e confirmar
  que persiste, sem afetar o `preferredKey` da música em "Minhas músicas".
- Adicionar uma terceira música a um setlist já salvo.
- Apagar uma música de "Minhas músicas" que está num setlist e confirmar
  que o setlist não quebra.
